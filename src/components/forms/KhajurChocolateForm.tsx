...
import { CalendarIcon, Plus, Calculator, Coffee } from 'lucide-react';
...
interface KhajurChocolateFormProps {
  onSubmit: (record: Omit<BillingRecord, 'id'>) => void;
}

export const KhajurChocolateForm: React.FC<KhajurChocolateFormProps> = ({ onSubmit }) => {
  ...
  const handleCalculate = () => {
    ...
    const calculated = calculateBillingFields(totalAmount, 'khajur-chocolate');
    ...
    setFormData(prev => ({
      ...
      khajurAmount: calculated.khajurAmount?.toString() || '',
      chocolateAmount: calculated.chocolateAmount?.toString() || ''
    }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const record: Omit<BillingRecord, 'id'> = {
      ...
      khajurAmount: parseFloat(formData.khajurAmount),
      chocolateAmount: parseFloat(formData.chocolateAmount),
    ...
    toast({
      title: "Success",
      description: "Khajur & Chocolate billing record added successfully",
      variant: "default"
    });
  };
    ...
    <Card className="bg-card border-border">
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>Add Khajur & Chocolate Record (GST: 12%)</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {isCalculated && (
            <div className="space-y-4 p-4 bg-secondary/30 rounded-lg border border-border">
              <h3 className="font-medium text-foreground mb-3">Calculated Fields (12% GST)</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="cgst">CGST (6%)</Label>
                  <Input
                    id="cgst"
                    type="number"
                    value={formData.cgst}
                ...
                <div>
                  <Label htmlFor="sgst">SGST (6%)</Label>
                  <Input
                    id="sgst"
                    type="number"
                    value={formData.sgst}
                ...
                <div>
                  <Label htmlFor="khajurAmount">Khajur Amount</Label>
                  <Input
                    id="khajurAmount"
                    type="number"
                    value={formData.khajurAmount}
                    onChange={(e) => setFormData(prev => ({ ...prev, khajurAmount: e.target.value }))}
                    className="mt-1"
                  />
                </div>
                <div>
                  <Label htmlFor="chocolateAmount">Chocolate Amount</Label>
                  <Input
                    id="chocolateAmount"
                    type="number"
                    value={formData.chocolateAmount}
                    onChange={(e) => setFormData(prev => ({ ...prev, chocolateAmount: e.target.value }))}
                    className="mt-1"
                  />
                </div>
              </div>
